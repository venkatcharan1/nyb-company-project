        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {categories.map((category, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className={`glass-card rounded-3xl p-8 border ${category.borderColor} bg-white/70 backdrop-blur-md space-y-6 relative overflow-hidden transition-all duration-300 hover:shadow-md hover:border-zinc-300`}
              >
                ...
                <p className="text-xs text-zinc-600 leading-relaxed">{category.desc}</p>
                <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-100">
                  {category.techs.map((tech, tidx) => (
                    <span
                      key={tidx}
                      className="px-3 py-1.5 rounded-full bg-zinc-55 border border-zinc-200 text-[10px] text-zinc-700 font-semibold tracking-wider uppercase bg-zinc-50"
                    >
                      {tech}
                    </span>
                  ))}
