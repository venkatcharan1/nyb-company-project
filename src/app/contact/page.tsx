...
export default function Contact() {
  ...
  return (
    <div className="w-full bg-white text-zinc-900 overflow-hidden min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12 mb-16 grid grid-cols-1 lg:grid-cols-3 gap-12 items-start reveal">
        {/* Info Column */}
        <div className="lg:col-span-1 space-y-8 text-left">
          <div className="glass-card rounded-3xl p-6 border border-zinc-200 bg-white/70 backdrop-blur-md space-y-6 shadow-sm">
            <h3 className="text-xl font-extrabold text-zinc-950">Contact Info</h3>
            <div className="space-y-4 text-sm text-zinc-600">
              <div className="flex gap-3 items-start">
                <MapPin className="w-5 h-5 text-indigo-600 shrink-0 mt-0.5" />
                <span className="font-medium leading-relaxed">Megrish Tower, Patrika Nagar, Madhapur, HITEC City, Hyderabad, Telangana 500081</span>
              </div>
              <div className="flex gap-3 items-center">
                <Mail className="w-5 h-5 text-indigo-600 shrink-0" />
                <a href="mailto:info@nybinfotech.com" className="hover:text-indigo-600 transition-colors font-semibold">info@nybinfotech.com</a>
              </div>
              <div className="flex gap-3 items-center">
                <Phone className="w-5 h-5 text-indigo-600 shrink-0" />
                <a href="tel:+914012345678" className="hover:text-indigo-600 transition-colors font-semibold">+91 40 1234 5678</a>
              </div>
            </div>
          </div>

          <div className="glass-card rounded-3xl p-6 border border-zinc-200 bg-zinc-50/50 space-y-3">
        ...
        <div className="lg:col-span-2">
          {state?.success ? (
            <div className="glass-card rounded-3xl p-8 md:p-16 border border-emerald-200 bg-emerald-50/50 text-center space-y-4 h-full flex flex-col justify-center items-center shadow-sm">
              <CheckCircle2 className="w-16 h-16 text-emerald-600" />
              <h3 className="text-2xl font-extrabold text-zinc-950">Message Received</h3>
              <p className="text-sm text-zinc-650 leading-relaxed max-w-md font-medium">
                {state.message}
              </p>
            </div>
          ) : (
