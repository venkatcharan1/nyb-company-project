...
export default function About() {
  ...
  return (
    <div className="w-full bg-white text-zinc-900 overflow-hidden">
      {/* Header */}
      <section className="max-w-7xl mx-auto px-6 py-16 text-center grid-bg">
        <span className="text-xs font-semibold uppercase tracking-wider text-indigo-600">Company Overview</span>
        <h1 className="text-4xl md:text-6xl font-extrabold text-zinc-950 tracking-tight mt-4">About Us</h1>
      </section>

      {/* Intro Grid */}
      <section className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-2 gap-12 text-left reveal">
        <div className="space-y-6">
          <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-950">Who Are We</h2>
          <p className="text-sm text-zinc-600 leading-relaxed">
            An individual character and arise as a regarded programming arrangements supplier by building and keeping up durable connections, conveying quality programming and by giving inventive business arrangements…
          </p>
        </div>
        <div className="space-y-6">
          <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-950">Our Mission</h2>
          <p className="text-sm text-zinc-600 leading-relaxed">
            To give inventive, high calibre and top tier IT Consulting & IT Solutions & Services to our clients, empowering them to accomplish their business destinations. Partners and accomplices while co-operating, and to maintain and grow our convention of “Greatness through Quality”.
          </p>
        </div>
      </section>

      {/* What We Do List */}
      <section className="max-w-7xl mx-auto px-6 py-16 text-left space-y-8 reveal">
        <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-950">What We Do</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={idx} className="glass-card rounded-xl px-5 py-4 border border-zinc-200 flex items-center gap-3">
      ...
      {/* Process list */}
      <section className="bg-zinc-50 border-y border-zinc-200/60 py-24 text-left reveal">
        <div className="max-w-7xl mx-auto px-6 space-y-12">
          <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-950">Our Process</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {processList.map((step, idx) => (
              <div key={idx} className="glass-card rounded-2xl p-6 border border-zinc-200 flex flex-col justify-between h-[220px]">
      ...
      {/* Why Choose Us */}
      <section className="max-w-7xl mx-auto px-6 py-24 text-left space-y-12 reveal">
        <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-950 text-center">Why Choose Us?</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {whyChooseUs.map((item, idx) => (
            <div key={idx} className="glass-card rounded-2xl p-6 border border-zinc-200 space-y-2">
      ...
      {/* Numbers Section */}
      <section className="bg-zinc-950 py-20 text-center text-white reveal">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-indigo-400 mb-2">Some Numbers</h2>
          <h3 className="text-2xl font-bold tracking-tight mb-12">Our Company in Numbers</h3>
      ...
      {/* Start Project CTA */}
      <section className="max-w-7xl mx-auto px-6 py-24 text-center reveal">
        <div className="glass-card rounded-3xl p-12 border border-zinc-200 space-y-6">
          ...
          <div className="flex justify-center">
            <Link
              href="/contact"
              className="px-8 py-3 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-all shadow-md text-sm"
            >
              CONTACT us
            </Link>
          </div>
